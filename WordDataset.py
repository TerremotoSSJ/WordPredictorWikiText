...
class TextLSTM:
    ...
    @staticmethod
    def split_article_into_sequences(article, vocabulary,sequence_length, step=None):
        ...
        :param sequence_length: Length of the sequences to be generated from the article
        ...
        :return:
            - current_word_targets: List of sequences of word indices representing the current words in the article, where each sequence is of length sequence_length and is generated by sliding a window over the article with the specified step size.
            - next_word_targets: List of word indices representing the next words in the article corresponding to each sequence in current_word_targets, where each index is the word that follows the corresponding sequence in the original article.
        """
        ...
        """
        We use the BPTT (Backpropagation Through Time) approach to generate sequences of words from the article, where we create overlapping sequences of words as input to the model and use the next word in the article as the target for each sequence. The step 
        parameter determines how much the window moves when creating new sequences, allowing for control over the number of sequences generated and the amount of overlap between them.
        """
...
class WordDataset(IterableDataset):
    def __init__(self,articles_generator, vocabulary,sequence_length, step=None):
        """
...
def collate_fn(vocabulary,batch):
        ...
        padded_next_word_targets = TextLSTM._pad_sequences(next_word_targets, max_length, vocabulary.pad_index)

        return torch.tensor(padded_current_word_targets), torch.tensor(padded_next_word_targets),torch.tensor(lengths_x),torch.tensor(lengths_y)
